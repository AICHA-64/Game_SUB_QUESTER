...
// レイと三角形の交差判定 (M?ller?Trumbore algorithm)
    ...
    float& v)
{
    ...
    XMVECTOR h = XMVector3Cross(dir, e2);
    ...
    t = f * XMVectorGetX(XMVector3Dot(e2, q));
    ...
    float maxDistance)
{
    ...
    {
        float t, u, v;
        if (RayTriangleIntersect(origin, direction, terrain->triangles[idx], t, u, v))
        {
