...
// 円形ミニマップ用ピクセルシェーダー
static ID3D11PixelShader* g_pPixelShaderCircle = nullptr;
...
bool Shader_Initialize(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	...
	// 円形ミニマップ用ピクセルシェーダーの読み込み
	std::ifstream ifs_ps_circle("resource/shader/shader_pixel_2d_circle.cso", std::ios::binary);
	if (ifs_ps_circle) {
		ifs_ps_circle.seekg(0, std::ios::end);
		std::streamsize filesize_circle = ifs_ps_circle.tellg();
		ifs_ps_circle.seekg(0, std::ios::beg);
		unsigned char* psbinary_circle = new unsigned char[filesize_circle];
		ifs_ps_circle.read((char*)psbinary_circle, filesize_circle);
		ifs_ps_circle.close();
		HRESULT hr2 = g_pDevice->CreatePixelShader(psbinary_circle, filesize_circle, nullptr, &g_pPixelShaderCircle);
		delete[] psbinary_circle;
		if (FAILED(hr2)) {
			hal::dout << "Shader_Initialize() : 円形ピクセルシェーダーの作成に失敗しました" << std::endl;
			// 続行は可能。g_pPixelShaderCircle は null のまま
		}
	} else {
		// ファイルが存在しない場合は通常シェーダーのみで続行
	}
...
{
	SAFE_RELEASE(g_pPixelShaderCircle);
...
// 円形ミニマップ用ピクセルシェーダーをセットする
void Shader_BeginCircle()
{
	// 頂点シェーダーは同じ
	g_pContext->VSSetShader(g_pVertexShader, nullptr, 0);
	// ピクセルシェーダーのみ円形版に切替（存在しなければ通常版）
	g_pContext->PSSetShader(g_pPixelShaderCircle ? g_pPixelShaderCircle : g_pPixelShader, nullptr, 0);
	g_pContext->IASetInputLayout(g_pInputLayout);
	g_pContext->VSSetConstantBuffers(0, 1, &g_pVSConstantBuffer0);
	g_pContext->VSSetConstantBuffers(1, 1, &g_pVSConstantBuffer1);
}
