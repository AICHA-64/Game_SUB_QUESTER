...
void ModelDraw(MODEL* model, const DirectX::XMMATRIX& mtxWorld)
{
	...
	for (unsigned int m = 0; m < model->AiScene->mNumMeshes; m++)
	{
		...
		{
			Texture_SetTexture(g_TextureWhite); // テクスチャをセット
			aiColor3D diffuse;
		...
		Direct3D_GetContext()->IASetIndexBuffer(model->IndexBuffer[m], DXGI_FORMAT_R32_UINT, 0); // 足りなかったのでR32に変えた

		// ポリゴン描画命令発行
		Direct3D_GetContext()->DrawIndexed(model->AiScene->mMeshes[m]->mNumFaces * 3, 0, 0); // 描画
	}
...
// シャドウマップ専用描画関数（深度のみ、ピクセルシェーダーなし）
