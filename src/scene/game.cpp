...
void GameScene::Initialize()
{
	...
	if (currentRound == 0 || isFinalBossBattle) {
		...
		// 2回目以降(通常戦)は位置固定、向きのみランダム
		playerSpawnPos = { 0.0f, 0.0f, -5.0f }; // 位置は固定
		...
		// 正規化
		float length = sqrtf(playerSpawnFront.x * playerSpawnFront.x + playerSpawnFront.z * playerSpawnFront.z);
		if (length > 0.001f) {
			playerSpawnFront.x /= length;
			playerSpawnFront.z /= length;
		}
	}
	...
	// 各層のパラメータを個別に設定
	// Layer 0: 広範囲の柔らかいBloom（通常）
	Bloom_SetLayerParameters(0, 0.1f, 0.6f);
	
	// Layer 1: ハイライト層（強い光源のみ抽出）
...
void GameScene::Update(double elapsed_time)
{
	...
	// ラスボス戦モードかどうかで終了条件が異なる
	if (Enemy_IsFinalBossMode()) {
		// ラスボス戦: ラスボスを倒したら通常戦に戻る（強化は保持）
	...
	if (m_IsGameFinished && Fade_GetState() == FADE_STATE_FINISHED_OUT) {
		if (Player::GetInstance().IsDead()) {
			...
			// ラスボス撃破 -> 通常戦に戻る（ドック経由でラウンド増加）
			Scene_Change(SCENE_DOCK);
		} else {
			// 通常ボス撃破 -> ドック(強化選択)へ
			Scene_Change(SCENE_DOCK);
		}
		return; // シーン変更後はこのフレームの更新を停止
	}
