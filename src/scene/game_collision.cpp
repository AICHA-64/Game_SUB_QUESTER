...
{
	// ラスボス戦モード:自機弾がラスボスに向かっている場合、敵が割り込む
	if (Enemy_IsFinalBossMode()) {
		...
		for (int i = 0; i < Bullet_GetBulletCount(); i++) {
			...
			// 弾の速度がない場合はスキップ
			XMVECTOR bulletVelVec = XMLoadFloat3(&bulletVel);
			float bulletSpeed = XMVectorGetX(XMVector3Length(bulletVelVec));
			if (bulletSpeed < 0.1f) continue;
			
			// 正規化
			XMFLOAT3 bulletDir;
			XMStoreFloat3(&bulletDir, XMVector3Normalize(bulletVelVec));
			
			// 弾からラスボスへのベクトル
			XMFLOAT3 toBoss;
			XMStoreFloat3(&toBoss, XMVectorSubtract(XMLoadFloat3(&finalBossPos), XMLoadFloat3(&bulletPos)));
			float distToBoss = XMVectorGetX(XMVector3Length(XMLoadFloat3(&toBoss)));
			
			// ラスボスまでの距離が遠い場合はスキップ
			if (distToBoss > 80.0f) continue;
			
			// 内積
			float dotProduct = XMVectorGetX(XMVector3Dot(XMLoadFloat3(&bulletDir), XMLoadFloat3(&toBoss)));
			...
			// 最短距離
			float closestDist = XMVectorGetX(XMVector3Length(
				XMVectorSubtract(XMLoadFloat3(&closestPoint), XMLoadFloat3(&finalBossPos))
			));
			
			// 自機弾がラスボスに当たる軌道でない場合はスキップ
			if (closestDist > bossCollisionRadius + 5.0f) continue;
			
			// 自機弾が現在位置からラスボスまでの距離のどこにいるか
			if (projectionLength > distToBoss + bossCollisionRadius) continue;
			
			// 迎撃地点を計算
			float interceptDistance = projectionLength * 0.7f;
			if (interceptDistance < 5.0f) interceptDistance = 5.0f;
			...
			for (int j = 0; j < Enemy_GetEnemyCount(); j++) {
				...
				if (normal && normal->IsGuardMode() && !normal->IsIntercepting()) {
					...
					float guardDist = XMVectorGetX(XMVector3Length(
						XMVectorSubtract(XMLoadFloat3(&guardPos), XMLoadFloat3(&interceptPoint))
					));
	...
	if (m_DamageInvincibleTimer <= 0.0) {
		...
		for (int i = 0; i < EnemyBullet_GetBulletCount(); i++) {
			...
			if (Collision_IsOverlapSphere(playerSphere, enemyBullet)) {
				...
				PlayAudio(m_minExSeId, false);
				m_DamageInvincibleTimer = 1.0; // 無敵
				break; // 1フレーム1回だけダメージ
			}
		}
	}
